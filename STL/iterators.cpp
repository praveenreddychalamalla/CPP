...
/*Iterators provide a means for accessing data stored in container classes like vector, map, list, etc.
  We can access the element by deferencing  the iterator with a *. just as you would with pointers
  Iterator types : Input Iterators, Output Iterators, Forward Iterators, Reverse Iterator, Bidirectional Iterators, Random-Access Iterators.
  class_name<template_parameters>::iterator name*/
int main(){
    vector<int>v{1,2,3,4};
    vector<int>::iterator it1; //'it1' is an iterator which can point to a vector of integers. Bidirectional Iterator - As 'it1' is created for container class vector
    vector<int>::reverse_iterator it2; //'it2' is a reverse iterator which can point to a vector of integers. Bidirectional Iterator- As 'it2' is created for container class vector
