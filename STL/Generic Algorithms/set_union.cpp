/**
    ...
    This code demonstrates the usage of set_union function.
*/
#include<bits/stdc++.h>
using namespace std;
/*
    Time Complexity - O(n).
    Note: set_union() works only on sorted ranges. Applicable to sorted containers like sets, maps, sorted arrays , sorted vectors etc.,
    ...
    set_union() Constructs a sorted range beginning in the location pointed by result with the set union of the two sorted ranges [first1,last1) 
    and [first2,last2).
*/
int main(){
    int a[]={5,10,15,20,25};
    ...
    sort(a,a+5);
    sort(b,b+5);
    auto it=set_union(a,a+5,b,b+5,v.begin());
    v.resize(it-v.begin());
    for(auto i:v)cout<<i<<" ";
    cout<<endl;
